...
var queryMap = {
  ...
  DICE: document.querySelector(".current-dice"),
  previous_DICE: document.querySelector(".previous-dice"),
  DICE_P0: document.querySelector(".player-0"),
  DICE_P1: document.querySelector(".player-1"),
...
function newGame(clicked = false, playAgain = false) {
  ...
  playAgain ^ clicked
    ? (players = initializePlayers(true))
...
function hold() {
  players[currentActivePlayer].score +=
    players[currentActivePlayer].currentScore;
  document.querySelector("#score-" + currentActivePlayer).innerHTML =
    players[currentActivePlayer].score;
  players[currentActivePlayer].currentScore = 0;
  document.querySelector(
    "#current-" + (currentActivePlayer ? 0 : 1)
  ).innerHTML = players[currentActivePlayer].currentScore;
  currentActivePlayer = currentActivePlayer ? 0 : 1;
  document
    .querySelector(".player-" + currentActivePlayer + "-panel")
    .classList.toggle("active");
  document
    .querySelector(".player-" + (currentActivePlayer ? 0 : 1) + "-panel")
    .classList.toggle("active");
}
...
function roll() {
  if (!game_is_on) {
    ...
    if (inputVal !== "" && !inputVal.startsWith("-") && !inputVal !== "0") {
      ...
      alert(
        "Wartość punktowa wymagana do wygrania rozgrywki musi zostać podana oraz musi być wartością niezerową!\nW przeciwnym razie zostanie wczytana domyślna wartość (100 punktów)."
      );
    }
  }
  if (
  ...
  if (previousToss === null) {
    queryMap.DICE_P0.classList.remove("hide-dice");
    queryMap.DICE_P1.classList.remove("hide-dice");
  } else {
    queryMap[`DICE_P${currentActivePlayer}`].setAttribute(
      "src",
      "dice-" + previousToss + ".png"
    );
  }
  ...
  document.querySelector("#current-" + currentActivePlayer).innerHTML =
    players[currentActivePlayer].currentScore;
  previousToss = toss;
  if (
    players[currentActivePlayer].score +
...
function playAgain() {
  var playAgain = confirm(
    "Player " +
      document.querySelector("#name-" + currentActivePlayer).innerHTML +
